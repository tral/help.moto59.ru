   ...
   /* class deliverReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent arg1) {
            ...
            case Activity.RESULT_OK:
            	setResDeliver(context.getString(R.string.info_sms_delivered));
                break;
            case Activity.RESULT_CANCELED:
            	setResDeliver(context.getString(R.string.info_sms_not_delivered));
                break;
            }
        }
    }
    ...
    public void run() {
        ...
        if (mState == STATE_RUNNING) {
	        ...
	        try {
	        	...
	        	//res_error = "EX6! " +  e.toString() +" Message:" +e.getMessage();
	        }
	        finally {
...
//			if ( deliver_receiver_fired==deliver_receiver_to_fire && send_receiver_fired==send_receiver_to_fire ) {
			if ( send_receiver_fired == send_receiver_to_fire ) {
		        unregisterReceivers(); // We need to unregister receivers to avoid potential memory leaks            
...
/*
    public void setResDeliver(String r) {
        res_deliver = res_deliver+r;
    }

	public void setDeliverRecToFire(int f) {
        deliver_receiver_to_fire = f;
    }

	public void setDeliverRecFired(int f) {
        deliver_receiver_fired = f;
    }
*/
